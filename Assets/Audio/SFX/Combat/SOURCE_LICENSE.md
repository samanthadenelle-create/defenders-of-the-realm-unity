...
the **same Pro-tier grant recorded in §2b**, which extends that section's coverage from 16 tracks to 21.
§2b's body is left as written (it is a dated record); this row is the extension.
...
| Installed path | Source file (owner drop) |
|---|---|
| `Assets/Audio/Resources/Music/heartwood_collapse.wav` | `Heartwood Collapse.wav` |
| `Assets/Audio/Resources/Music/maple_spellbook.mp3` | `maple-spellbook.mp3` |
| `Assets/Audio/Resources/Music/echoes_beneath_elarion.mp3` | `echoes-beneath-elarion.mp3` |
| `Assets/Audio/Resources/Music/grove_against_darkness.mp3` | `Grove Against Darkness.mp3` |
| `Assets/Audio/Resources/Music/harp_victory.mp3` | `Harp Victory.mp3` |

Files were renamed to the folder's lowercase-underscore convention (matching the sibling
`Music/echo_theme.mp3`); contents are byte-identical to the owner's drops. **Not yet assigned to any
music slot** — which track plays where is an open owner decision, so none is wired in
`AudioBootstrap.cs` yet. They ship in the build (they are under a `Resources/` folder) regardless of
wiring.
