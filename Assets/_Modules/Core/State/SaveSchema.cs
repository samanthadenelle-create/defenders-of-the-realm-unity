...
namespace DeNelle.Core.State
{
    ...
    public static class SaveSchema
    {
        ...
        public sealed class PersistedState
        {
            ...
            /// <summary>
            /// Per-Echo gather-lane CSV by echo index ("wood,iron,idle"; lane ids
            /// wood/iron/food/idle). Written by the WO-681 Echo card; WO-658's rate
            /// split consumes the same field. Nullable per the <c>.partial()</c>
            /// convention; absent on an older save → GameState's initializer default
            /// ("wood" — the auto-assigned starter Echo) applies on read, so no
            /// migrator step is required. Append-only field at the END so older
            /// saves stay loadable.
            /// </summary>
