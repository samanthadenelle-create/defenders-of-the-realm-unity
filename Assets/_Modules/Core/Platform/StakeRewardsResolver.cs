...
namespace DeNelle.Core.Platform
{
    ...
    public static class StakeRewardsResolver
    {
        ...
        // =====================================================================
        //  Table load (owner-adjustable JSON, hardcoded fallback)
        // =====================================================================

        private static void EnsureTable()
        {
            if (_tiers != null) return;
            _tiers = LoadFromJson() ?? DefaultTiers();
        ...
        private static List<StakeTier> LoadFromJson()
        {
            ...
            {
                FlowTrace.Warn("Stake", $"stake-rewards.json read threw ({ex.Message}) — using hardcoded default tiers.");
                return null;
            }
            if (string.IsNullOrEmpty(json))
            {
                FlowTrace.Warn("Stake", "stake-rewards.json not found — using hardcoded default tiers.");
                return null;
            }
            ...
            try
            {
                ...
                {
                    FlowTrace.Warn("Stake", "stake-rewards.json has no 'tiers' array — using hardcoded default tiers.");
                    return null;
                }
            ...
            {
                FlowTrace.Warn("Stake", $"stake-rewards.json parse failed ({ex.Message}) — using hardcoded default tiers.");
                return null;
            }
        }
        ...
        /// <summary>
        /// The hardcoded default ladder — MIRRORS stake-rewards.json so the surface never boots empty.
        /// Rewards are deliberately SMALL ("small but something"): a token thank-you, never pay-to-win.
        /// The owner tunes the live values in the JSON; keep these in step if the JSON schema changes.
        /// </summary>
        private static List<StakeTier> DefaultTiers()
        {
            _currencySymbol = "SKR";
            return new List<StakeTier>
            {
                ...
                {
                    new StakeReward("Title: \"Vanguard\"", "A cosmetic name title.", StakeRewardKind.Title, "vanguard", "Vanguard"),
                    new StakeReward("Crystal Dust", "+10 Crystals per day — a small trickle.", StakeRewardKind.Trickle, "vanguard", "Vanguard"),
                }),
                new StakeTier("warden", "Warden", 25000, "The realm marks you a Warden.", new[]
                {
                    new StakeReward("Warden Banner Sigil", "A cosmetic banner sigil flourish.", StakeRewardKind.Cosmetic, "warden", "Warden"),
                }),
                new StakeTier("genesis", "Genesis", 250000, "A founding stake of the realm.", new[]
                {
                    new StakeReward("Title: \"Genesis\"", "The founding-staker cosmetic title.", StakeRewardKind.Title, "genesis", "Genesis"),
                    new StakeReward("Genesis Aura Flourish", "A small golden aura flourish on your hero in town — purely cosmetic.", StakeRewardKind.Cosmetic, "genesis", "Genesis"),
                }),
            };
        }
