...
| `GooglePlay/` | `DeNelle.GooglePlay` | **The Google Play artifact's whole store and identity rail** (added to this table 2026-09-06; the module existed undocumented). Three files: `GooglePlayStorefront` (the **ONE** `PanelId.RealmStore` registrar in a Play build), `GooglePlayStorefrontVM`, `GooglePlayIdentityClient` (Google Sign-In). ⛔ **It and `DeNelle.Wallet` can NEVER coexist in one artifact and that is by construction, not by convention:** this asmdef carries `defineConstraints: ["GOOGLE_PLAY"]` and `DeNelle.Wallet.asmdef` carries `["!GOOGLE_PLAY"]` (WO-1282 Lane B, commit `c06a66de5`), so exactly one storefront registrar compiles into any build and the Night Market does not exist in the Play artifact at all — read `GooglePlayStorefront.cs`'s header, which records WO-1395 being minted on a static-init race premise that was then **disproven at source**. Also `includePlatforms: ["Android","Editor"]`. References `DeNelle.Core`, `DeNelle.Commerce`, `DeNelle.PaymentProviders.GooglePlay`, `GoogleSignIn`, `UnityEngine.UI`, `Unity.TextMeshPro` — **never `DeNelle.Wallet`**, though its `.cs` may `using DeNelle.Wallet` for `PackCatalog`'s preserved NAMESPACE (the type lives in `DeNelle.Commerce`; see the `Commerce/` row) |
