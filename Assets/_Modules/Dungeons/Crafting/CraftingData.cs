    ...
    public static class CraftingDataLoader
    {
        ...
        public static async UniTask<CraftingDataSet> LoadAsync()
        {
            ...
            string fullPath = Path.Combine(
                Application.streamingAssetsPath, CraftingDataRelativePath);

            string json = await ReadTextAsync(fullPath);
            if (string.IsNullOrEmpty(json))
            {
                Debug.LogError($"[CraftingDataLoader] Could not read '{fullPath}'.");
                return null;
            }
        ...
        /// inside the compressed APK. Same caveat as <see cref="LoreFragmentsLoader"/>.
        /// </summary>
