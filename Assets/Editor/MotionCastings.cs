...
namespace DeNelle.Editor
{
    ...
    public static class MotionCastings
    {
        ...
        private static AnimationClip LoadClipAtPath(string path)
        {
            ...
            if (direct != null && !direct.name.StartsWith("__preview", StringComparison.Ordinal))
                return direct;
            // FBX container: pick the first non-preview clip sub-asset.
            foreach (var a in AssetDatabase.LoadAllAssetsAtPath(path))
                if (a is AnimationClip c && !c.name.StartsWith("__preview", StringComparison.Ordinal))
                    return c;
            return null;
        }
    }
