// =============================================================================
// TalentPointerVfxMirror - ships the owner-picked talent node POINTER effect.
// -----------------------------------------------------------------------------
...
//   -> the TALENT TREE NODE POINTER (HeroSkillTreePanelMvvm focus node; runtime
//      rig = DeNelle.Village.Talents.TalentNodeVfxRig.CreatePointer()).
//
...
// any machine without the pack). This utility therefore does BOTH steps:
//
//   STEP 1  CopyAsset the prefab into Assets/Resources/VFX/UI/TalentNodePointer
//           .prefab (idempotent - an existing mirror is ADOPTED, never re-copied,
//           which is what preserves its GUID across runs).
//   STEP 2  Run the sanctioned self-containment pass, VfxResourceArtMirror.Run()
//           - the SAME utility that self-contained the 27 prefabs on 2026-08-06.
//           It scans EVERYTHING under Assets/Resources/VFX/, so the new mirror is
//           in scope automatically: pack demo scripts are stripped, materials/
//           textures/shaders are mirrored into _Shared/ and re-pointed.
...
//
// RUN:
//   Editor menu : Defenders/VFX/Mirror Talent Pointer VFX
//   Batchmode   : DeNelle.Editor.TalentPointerVfxMirror.Run
//   Markers     : TALENT_POINTER_VFX_OK / TALENT_POINTER_VFX_FAIL
...
{
    /// <summary>
    /// Copies the owner-picked Hovl "Marker 2 Pointer Loop" prefab into the tracked
    /// Resources/VFX tree and self-contains its art via VfxResourceArtMirror. Idempotent.
    /// Prints TALENT_POINTER_VFX_OK only when the mirrored prefab measures ZERO
    /// remaining gitignored-pack dependencies.
    /// </summary>
    public static class TalentPointerVfxMirror
    {
        private const string MarkerOk   = "TALENT_POINTER_VFX_OK";
        ...
        public static void Run()
        {
            try
            {
                ...
                else
                {
                    if (AssetDatabase.LoadAssetAtPath<GameObject>(SrcPath) == null)
                        throw new Exception("source prefab would not load: '" + SrcPath +
                            ...
                            "pointer cannot be mirrored here. Import the pack and re-run; nothing was faked.");

                    EnsureFolder("Assets/Resources/VFX/UI");
                ...
                Debug.Log(MarkerOk + " - '" + DstPath + "' mirrored (packDeps " + before +
                          " -> 0); runtime Resources path 'VFX/UI/TalentNodePointer' now resolves " +
                          "on a fresh clone.");
            }
            catch (Exception e)
            {
