...
| `Data/` `Resources/` `Settings/` `Localization/` `StreamingAssets/` | Config + runtime-loaded assets. ⚠ **Canonical gameplay JSON is DUAL-COPIED** — every file exists at BOTH `Resources/Data/Canonical/` and `StreamingAssets/Data/Canonical/`, and edits must land in both or the two paths silently disagree (recent additions: `dungeon-balance.json`, `jewel-polish.json`). Copies under `Builds/` and `Library/Bee/` are build artifacts — never a source of truth. `Resources/Collectors/` holds `CollectorStackPropCatalog.asset`, the collector prop catalog behind `_Modules/Village/Buildings/Progression/CollectorStackPropCatalog.cs` |
