...
| `Editor/` | `DeNelle.Editor` asmdef. Scene builders — `VillageSceneBuilder` (partial, split across ~10 files; **serialization bottleneck, one agent at a time**), `BattleSceneBuilder`, `DungeonSceneBuilder`, `ExteriorTerrainBuilder`, `FolksGranaryBuilder`, `IntroFlowSceneBuilder` — plus animator setups/factories, build scripts (`AndroidBuild`, `DesktopBuild`), `CompileGate`, material fixers (`KayKitMaterials`), portrait generators |
