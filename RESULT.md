...
**B. ✅ DONE — Tree-of-Life collider fixed.** The plaza wasn't blocked by a mesh collider (there was none) — `HeartController` sits on the scale-**11** `tree_of_life` mesh, so its runtime blocker capsule (r=2, h=8 *local*) inflated to a **22 m-radius, 88 m-tall** world blocker that swallowed the whole plaza. Baked a scale-compensated **trunk capsule** into `Village.unity` (local r=0.18 h=0.73 = world ~2 m × 8 m, feet at ground); the runtime auto-blocker now early-returns instead of re-inflating. Enemies hit the Heart by proximity (not a collider), so combat is unchanged — the hero just stops at the actual trunk now. Verified: collider count 61→62, capsule serialized correct, Windows build SUCCEEDED (542 MB, no `level3` regression). Committed `50bba17`.
