...
- `SwapEnemy()`: loads `Resources/Enemies/<ResolveEnemySlug()>` (reads the live `SceneRouter.PendingBattle.BreachedIds` first breacher, F-SWAP-2 RESOLVED), faces hero via `LookRotation`, stamps shared enemy animator controller (`EnemyControllerFor` map), `TripoMaterialFixer` by reflection; falls back to violet `TintEnemy` if no model.
...
**MonoBehaviour singleton** (`Instance`, `[DisallowMultipleComponent]`). **Turn idle-timer** (WO-68/93): counts up while combat active + player turn; on timeout (`maxTurnTime=8s`) fires `onEnemyAutoAttack` UnityEvent. **WIRED/LIVE** — BattleController calls `StartCombat`/`OnPlayerActed`/`StopCombat` AND subscribes `HandleIdleTimeout` to `onEnemyAutoAttack` in code (F-MGR-1 RESOLVED); `onPlayerTurnStart` still has no listener. `TurnProgress`/`IsActive` exposed for HUD but `TickVisualAtb` is what actually drives the bars.
...
- **F-MGR-1 (RESOLVED):** the idle-timer is now wired — `BattleController.Start()` subscribes `HandleIdleTimeout` to `ATBCombatManager.onEnemyAutoAttack` (BattleController.cs:179-180, guarded RemoveListener→AddListener) before `StartCombat()`, so on idle-past-`maxTurnTime` the active hero is forced to Defend through the normal engine path (WO-93). (`onPlayerTurnStart` still has no listener; `TurnProgress` is still unused — HUD ATB uses `TickVisualAtb`.)
...
- **F-SWAP-2 (RESOLVED):** `AtbCombatantSwapper.ResolveEnemySlug()` (AtbCombatantSwapper.cs:499-524) now reads the live handoff `SceneRouter.PendingBattle.BreachedIds` — it maps the FIRST breacher's id → a `Resources/Enemies` model slug (hollow-warrior→Skeleton_Warrior, hollow-walker→Skeleton_Minion, hollow-rogue→Skeleton_Rogue, hollow-acolyte→Skeleton_Healer, else `ModelSlugForEngineDef(EngineDefFor(id))`). With no handoff (dev/direct-play) it varies deterministically by wave. No longer a hard-coded constant.
...
### Contradictory / risk
- **F-WAVE-1 (RESOLVED):** `BattleHudUgui.Render` now sets `_waveText.text = "WAVE " + Mathf.Max(1, state.Wave)` (BattleHudUgui.cs:830) — the real, scaled `BattleState.Wave` drives the label (floored at 1 so a dungeon/dev Wave-0 battle reads "WAVE 1", not "WAVE 0"). No longer hard-coded.
