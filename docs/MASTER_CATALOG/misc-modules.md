...
| **Composed (RoomForge)** | `Assets/Scenes/DungeonCompose/dg_starter_loop.unity` — **BINARY-serialized** (header bytes non-printable, verified 2026-08-02; 175 KB) — the DGN-1 scene. NO re-save from a shared tree (memory: dungeon-scene-shared-tree-corruption; re-bake only in an isolated worktree). | Baked from `StreamingAssets/Data/Canonical/dungeon-layouts/<id>.json` (`DungeonComposeLayout`) by the **editor** RoomForge stack | **NO DungeonController** — root `DungeonCompose_*`; play-population by the editor baker; `DungeonExitSpawner` injects the way home at load | live-Enemy hero-aggro spawners (bake-time) | `DungeonExitSpawner`-injected `DungeonExitInteractable` → `SceneRouter.Castle` |
