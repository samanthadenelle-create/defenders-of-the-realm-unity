...
| `SaveSchema` (static) + `SaveFile`/`PersistedState`/`SaveValidationException`/`SaveValidationResult` | `State/SaveSchema.cs` | Save shape + validator (Zod port). `CurrentVersion = 20`, `FileFormat = 1`, key `dotr-save`. `PersistedState` = all-nullable 41+ fields ([JsonProperty] camelCase). `Validate` (NonNegInt/FiniteInt clamps, NaN/Inf reject). `JsonSettings` (StringEnumConverter + TutorialStepConverter). | — | LIVE; FLAG header says v10 (see FLAGS) |
| `SaveMigrator` (static) + `SaveMigrationResult` | `State/SaveMigrator.cs` | Registry-based v1→v20 migration. Steps dict (2-10,14,17,18; 11-13,15,16,19,20 are additive-default-on-read, no step). `Migrate`/`MigrateForImport` (rejects newer-than-build). | — | LIVE; FLAG header says "v1→v10 / nine-step" (stale) |
...
| `RpgUiCatalog` (static) | `UI/RpgUiCatalog.cs` | WebGL-safe RPG-UI sprite-pack accessor (Resources/RpgUi/<role>). Sprite-or-null contract. | LIVE (null until pack imported) |
| `ShopTheme` (static) | `UI/ShopTheme.cs` | WO-175 shared shop palette + UI-Toolkit helpers (CosmeticShop + PackStore). | LIVE theme |
...
`SaveLoadRoundTripTest` (41-field round-trip + simulated restart), `ResetCarveOutTest` (ResetToNewGame preserves wallet/breachStyle/social), `SaveMigratorTest` (v1→v10 step chain + version gate — note: header says v1→v10, schema is now v20), `SaveSchemaValidateTest` (NaN/Inf reject + clamps), `TestSupport` (SpawnService/ClearSave/MakeRichState/WriteSaveFile via reflection). asmdef refs DeNelle.Core + **DeNelle.Data** (note: not DeNelle.Core.Data namespace — a separate `DeNelle.Data` asmdef).
...
1. **`SaveSchema.cs` header** (lines 13-14) says "CurrentVersion = 10, FileFormat = 1" — actual `CurrentVersion = 20` (FileFormat = 1 is still correct). The inline doc on the const IS up to date; the file banner is stale.
2. **`SaveMigrator.cs` header** describes "the migration chain v1 → v10 (nine stacked-if)" and "nine-step" — the chain now runs to **v20** (steps through 18 + several additive-default-on-read versions). Banner stale; the Steps dict + step XML are current.
...
5. **`SaveMigratorTest` header** ("v1→v2 ... v9->v10", "nine-step") — stale like #2 (test still validates the real chain but the doc count is wrong).
...
`GameState.Tribes`, `Settlements`, `Wards`, `Arena`, `PetName` — live correctly in-memory within a session but the schema round-trip + version bump is an explicitly-deferred save-owner follow-up (per the field XML docs). `Zones`/`BaseLayout`/`PartyMemberIds`/`ArenaDefense` ARE wired (v17/v14/v16/v19).
