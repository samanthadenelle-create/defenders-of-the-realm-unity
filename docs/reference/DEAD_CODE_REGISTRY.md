...
| 19 | **Gear lore never reaches the player** | `GearAppraisal.cs:56 FullText`, `:149`, `:152` | a tooltip / vendor-detail panel calling `FullText()` | `Appraise()` **is** live (`GearCatalog.cs:508,521,528`; `ShopVM.cs:352,368,389,433,454,475`) — only the multi-line lore text is unused, so the WO-300 Elarion maker's-mark lore (Emberhand / Oathweld / Heartwood / Last-Pressing) is invisible. Only the price shows. |
...
| `GearCatalog.AllAccessories()` (`:468`) | `Accessories` (`:458`) — its own docstring calls it an alias; production uses the property (`ShopVM.cs:319-320`). Repoint `ArmedHeroInvariantRegression`. |
