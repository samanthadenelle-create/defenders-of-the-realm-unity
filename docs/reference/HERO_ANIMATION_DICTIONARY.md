...
> ## VERIFIED 2026-09-06 (Sunday sweep, step 5) - ten-row spot-check, and it found real drift
>
> **Age when checked:** body header dated 2026-07-19 (49 days); last git commit
> `Sun Aug 16 18:09:34 2026` (21 days). Both are past the 7-day tripwire
> (`SUNDAY_HOUSEKEEPING.md` section 4), so every row below was re-opened at source.
>
> | # | Claim checked | Verdict | Source read 2026-09-06 |
> |---|---|---|---|
> | 1 | `ff.knightv3` default ON | VERIFIED (line moved 463 -> 591) | `Assets/_Modules/Core/FeatureFlags.cs:591` |
> | 2 | `ff.mocaploco` default ON | VERIFIED (line moved 476 -> 604) | `Assets/_Modules/Core/FeatureFlags.cs:604` |
> | 3 | KnightV3 + mocaploco binds `KnightMocap.controller` | VERIFIED (line moved 472-486 -> 586-603) | `Assets/_Modules/Village/Hero/HeroBodySwapper.cs:601-603` |
> | 4 | Controller is code-generated by `BuildKnightMocapController` | VERIFIED (line moved 300 -> 354) | `Assets/Editor/HeroAnimatorFactory.cs:354` |
> | 5 | "Knight W/E/R carry no `id`; only Q does" | **STALE - FIXED** | `abilities.json:95,111,127,147` = `knight.q/w/e/r`; kit `_comment` at `:92` |
> | 6 | W is named "Shield Charge", 2 mana | **STALE - FIXED** | renamed **Shield Bash** by owner ruling 2026-08-21, manaCost **3** (`abilities.json:111-125`) |
> | 7 | E "Warden's Grace" is `effect=taunt`, cd 12 | **STALE - FIXED** | `effect=gracebuff`, cd **20** - redesigned to heal + Grace Shield (`abilities.json:127-145`; `"cooldown": 20` and `"effect": "gracebuff"` read as fields, not from the kit comment) |
> | 8 | R plays `CastVariant=2`, the same clip as W | **STALE - FIXED** | R castAnim `r` -> **CastVariant 4**, its own `Cast_r` state, hardcoded `MocapSpellClips[4]` (`motion-castings.json:3`) |
> | 9 | knight `skill1`/`skill2`/`castHeal` clips | **STALE - FIXED** (line refs 166/181/203 still correct) | `motion-castings.json:167` `atk_jump.fbx`, `:182` `atk_shieldcharge.fbx`, `:204` `f-ss-magespellcast-05.fbx` (vfxKey `Heal_Cast`, not `Dash_Blink`). **BAKE ALSO PROVEN, not just the registry:** all three guids are present in the baked controller - `grep -c` in `Assets/Resources/Heroes/KnightMocap.controller` returns 2 for each of `8d75c257eb9328d41bfbcd013dfe1935` / `6aef29d5373a1ff44b33b2bc3db5f1d6` / `d2be6fe2ab0d3704ca777bbb5c48e378`, so the section-1/3 "on-disk verified" wording stands. |
> | 10 | Basic attack damage `_baseDamage` = 30 at `:36` | **STALE - FIXED** | `PlayerAttackController.cs:43` = **52.5** (the x1.75 perfect-hit was folded in; see the in-file note at `:152`) |
> | 11 | `Attack` trigger + `Combo` int in `ActorAnimator` | VERIFIED, line-exact | `Assets/_Modules/Core/Combat/ActorAnimator.cs:132-133` |
> | 12 | Hot-swap row built at `HudKitController.cs:686-719` | **STALE line - FIXED** (fact true) | `BuildAssignableSkillRow` at `Assets/_Modules/HUD/Kit/HudKitController.cs:2506`, called at `:735` |
> | 13 | Hot-swap PlayerPrefs key `dotr-skillbar-extra-v1` | VERIFIED | `Assets/_Modules/Village/Hero/AssignableSkillBar.cs:26` |
> | 14 | Addendum: `ranger.q` "Quick Shot", verb "Shoot", icon `Hunter12` | VERIFIED | `abilities.json:177,180,181`; `concept-icons.json:91` |
>
> **NOT re-verified this pass** (out of the ten-row scope, still carrying their 2026-07-19 dates):
> the OPEN GAPS section's stale-bake finding (gap 1), the 16-id hot-swap pool, and every
> section-3 locomotion/death clip row. Treat them as UNPROVEN, not wrong.
>
> **Standing risk this pass exposed:** OPEN GAP 4 ("mint `knight.w/e/r`") was CLOSED in the tree
> and stayed open in this dictionary for weeks. Gap sections go stale the same way line numbers do.

# Hero "Grom" (Knight) — Right ActionBar, Hot-Swap Bar & Full Animation Audit

**Live hero:** single Tripo self-rigged Knight **"Grom"** (class id `knight`, KnightV3 CC/AccuRIG body). Runtime body/animator binding chain: `HeroClass.Knight` → `ff.knightv3` ON (default, `FeatureFlags.cs:591`) → loads `Resources/Heroes/KnightV3.fbx` → `ff.mocaploco` ON (default, `FeatureFlags.cs:604`) binds **`Assets/Resources/Heroes/KnightMocap.controller`** (`HeroBodySwapper.cs:586-603`), falling back to `Knight.controller` only if the mocap load misses. `applyRootMotion=false`, `anim.speed=1.0`.

The animator is **code-generated** by `DeNelle.Editor.HeroAnimatorFactory.BuildKnightMocapController` (`HeroAnimatorFactory.cs:354`), NOT hand-authored. Per-ability cast/skill/locomotion clips are owner-picked in `motion-castings.json` (target `knight`) and baked in via `MotionCastings.Resolve` at bake time. The Paladin lane (`KnightPackageControllerBuilder` + `weaponskill-animations.json`) is **DEAD/gated-out** for Grom (`ff.knightv3` cuts off its only consumer) — do NOT source Grom clips from `weaponskill-animations.json`.
...
|---|---|---|---|---|---|---|---|
| **ATTACK** (pill) | *(none — no ability id/JSON row)* | **Sword Wielding** (basic sword melee; owner-named 2026-07-19) | 3-swing melee combo (`ComboLength=3`, cycles 0→1→2), **dmg = `_baseDamage` **52.5** (STALE-FIXED 2026-09-06: the x1.75 perfect-hit bonus was FOLDED IN, 30 x 1.75 = 52.5)** × weaponMult (RETIRED 2026-09-06: the ×1.75 perfect-hit multiplier is FOLDED INTO the 52.5 and is NOT applied again — `PlayerAttackController.cs:152,646`; ×3.0 riposte still applies), reach ~3.2m, cd 0.6s. Gated `BattleLock.IsInBattle` + `InputSuppressed`. | ATTACK pill (bottom-right, `Pill611` rect) → `HudCommands.Attack` → `TriggerBasicAttack`. Also Space / LMB / gamepad-South via `PlayerAttackController.StartAttack` | `Attack` trigger + `Combo` int (`ActorAnimator.PlayAttack`, `ActorAnimator.cs:132-133`) | Combo0 = **`atk_slashright.fbx`** (guid cdc0b23…, on-disk KnightMocap Attack0) → Combo1 `atk_slashleft.fbx` → Combo2 `atk_spin.fbx`. *(Motion-castings humanoid-inherited attack0 `Combat_Weapon_WeaponSkill_Sword_And_Shield_Slash.anim` is dormant for KnightMocap.)* | **none on basic hit** (owner 2026-08-02 — `Weaponskillsword_Impact`/`KnightWeaponskill_Impact` deleted from the melee path); PERFECT hit = floating gold **PERFECT** stamp (`CombatText`); elemental brand burst only on element weapons (`WeaponVfxMap.ElementalOnHitKey`). `GameSfx.PlaySwordSwing` audio unchanged |
| **SKILL 1 / Q** | **`knight.q`** | **Sword Heroic** | effect=`dash`. Warps to reticle-locked/nearest in-range hostile (stops 1.6m short), **30 dmg** (Aether) + `Freeze` 1.0s stun. Falls back to live boss else whiffs. **range 10, cd 6s, 0 mana.** | LMB / Space / gamepad-South (primary) **and** Q medallion (slot 0) → `AbilityRequested.Invoke(0)` → `TryCast(Q)`. Gated `BattleLock.IsInBattle`. | `Cast` trigger + `CastVariant`=1 (`PlayCast`), plus `PlayAttack(0)` melee overlay. animVariant: dash→`leap`→unmapped(-1)→slot-fallback = **1** | `CastVariant=1` = knight `skill1` = **`atk_jump.fbx`** (`motion-castings.json:166`, on-disk verified) | Registry-only: cast `skill1` vfxKey='' (**silent**); impact `Melee_Impact`. (abilities.json `Dash_Blink`/`Melee_Impact` suppressed) |
| **SKILL 2 / W** | **`knight.w`** *(STALE-FIXED 2026-09-06: minted; no longer slot-addressed only)* | **Shield Bash** *(RENAMED by owner ruling 2026-08-21 from "Shield Charge"; mechanic unchanged. manaCost is **3**, not 2)* | effect=`knockback`. Forward cone (centre = origin+fwd×range×0.5, radius range+0.85): **26 dmg** + `Slow` 1.5s + `Freeze` 0.4s (guard-break) + 6-unit knockback impulse. **range 4.5, cd 9s, 2 mana.** | W medallion (slot 1) → `AbilityRequested.Invoke(1)` → `TryCast(W)`. Gamepad-East. HUD label "W" (JSON `key`=F). | `Cast` + `CastVariant`=2, plus `PlayAttack(0)` overlay. animVariant: knockback→`slam`→unmapped(-1)→slot-fallback = **2** | `CastVariant=2` = knight `skill2` = **`atk_shieldcharge.fbx`** (`motion-castings.json:181`, on-disk verified) | Registry `skill2` vfxKey='' (**silent** cast); impact `Cleave_Impact` |
| **SKILL 3 / E** | **`knight.e`** *(STALE-FIXED 2026-09-06: minted)* | **Warden's Grace** | **effect=`gracebuff`, cd 20** (STALE-FIXED 2026-09-06 - the taunt was REDESIGNED into a hybrid support: heal 25% max HP read from `damage` as a percent + Defense-scaled bonus, then a Grace Shield HoT 5% max HP/2s and -20% incoming. The old text below describes the RETIRED taunt). `Slow` for max(2, freeze=4)s to every hostile within range+0.85; **0 direct dmg**; grants Knight **+20 HP** stand-in shield. **range 6, cd 12s, 3 mana.** | E medallion (slot 2) → `AbilityRequested.Invoke(2)` → `TryCast(E)`. Gamepad-West. HUD label "E". | `Cast` + `CastVariant`=3, plus `PlayAttack(0)` overlay. animVariant: taunt→`shout`→unmapped(-1)→slot-fallback = **3** | `CastVariant=3` = knight `castHeal` = **`f-ss-magespellcast-05.fbx`** (Magic Spell Cast gesture, `motion-castings.json:203`, on-disk verified). ⚠ mismatched borrow — a taunt plays a heal-channel clip | Registry `castHeal` vfxKey=`Dash_Blink`, sfx `Heal`; held `Taunt_Aura` residual loop for freeze-seconds |
| **ULT / R** | **`knight.r`** *(STALE-FIXED 2026-09-06: minted)* | **Radiant Strike** | effect=`meteor`. `castSeconds=0.5` interruptible wind-up (moving cancels+refunds); launches projectile to nearest cluster then Blast(radius=range) **220 dmg** on arrival. **blast range 9, cd 40s, 6 mana.** | R medallion (slot 3) → `AbilityRequested.Invoke(3)` → `TryCast(R)`. Gamepad-North. HUD label "R". | `Cast` + `CastVariant`=**4** (STALE-FIXED 2026-09-06: R now has its OWN `Cast_r` state via castAnim `r`; it no longer shares W's clip. The old "variant 2 overrides pressed-slot 4" is retired), plus `PlayAttack(0)` overlay | `CastVariant=4` = `Cast_r` = hardcoded `HeroAnimatorFactory.MocapSpellClips[4]` (NOT registry-resolved; `motion-castings.json` line 3 says so explicitly). STALE-FIXED 2026-09-06 - it is no longer W's clip | Cast VFX keyword for variant 4 = **null → SILENT by design**; meteor-landing impact `Cleave_Impact` |

**Source:** `abilities.json:92` (the knight kit `_comment`) + the four id rows `:95` `knight.q` / `:111` `knight.w` / `:127` `knight.e` / `:147` `knight.r` - STALE-FIXED 2026-09-06: **ALL FOUR carry ids**; the old "only Q has an id" is retired; `HeroAbilities.cs:400-448,562-643,1354,1369-1443`; `HudKitController.cs:377-393,623-684`; `HudKitCommandBridge.cs:58-64`; `PlayerAttackController.cs:43,70,177,368-419,540`; `motion-castings.json:163-227`.

---

## 2. HOT-SWAP BAR (AssignableSkillBar)

**What it is:** a **BUILT** 4-slot player-assignable quick-slot bar — `AssignableSkillBar.cs` (index→abilityId map, PlayerPrefs `dotr-skillbar-extra-v1`, edits battle-locked). Rendered bottom-CENTER as `AssignableSkillRow` (`HudKitController.cs:2506` (`BuildAssignableSkillRow`; was 686-719)) firing `HudCommands.AssignableCast(slot)` → `HeroAbilities.TryCastExtra(abilityId)`. **It holds ABILITIES/SKILLS ONLY** — NOT weapons, NOT potions/consumables (those are separate fixed slots), NOT stances, NOT echoes. Slots are filled out-of-combat from the Skill-Tree loadout chooser. The chooser filter (proven complete + exclusive) exposes **exactly 16 placeable ids for a Knight**: 13 `knight-skills` + 3 `universal-skills`. `ranger.*`/`mage.*` stubs and base Q/W/E/R are excluded by construction and by `FindById` (no resolving def).
...
|---|---|---|---|---|---|
| `knight.ranged-poke` | Throwing Spear | knight-skill / strike | 28 dmg, 16m, 1.2s cd, 1 mana, 0.35s wind-up | `PlayCast(1)` (Cast + CastVariant=1) | Cast_q / `skill1` = `atk_jump.fbx` |
| `knight.mending-salve` | Mending Salve | knight-skill / heal | 42 HP self/Heart, 5m, 16s cd, 4 mana | `PlayCast(3)` | Cast_e / `castHeal` = `f-ss-magespellcast-05.fbx` |
| `knight.snare-arrow` | Snare Arrow | knight-skill / snare | 18 dmg + 2.5s slow, 14m, 11s cd, 3 mana | `PlayCast(1)` | Cast_q / `skill1` |
| `knight.suppressing-volley` | Suppressing Volley | knight-skill / cleave | 36 dmg 6m arc, 20s cd, 5 mana | `PlayCast(2)` | Cast_w / `skill2` = `atk_shieldcharge.fbx` |
...
**Source:** `AssignableSkillBar.cs:33-140`; `HudKitController.cs:2506` (`BuildAssignableSkillRow`; was 686-719); `HeroAbilities.cs:459-496,733-756`; `abilities.json:86-113`; `hero-talents.json:14-35,125-127`; `HeroLoadoutVM.cs:243-267`; `HeroSkillTreeVM.cs:698-712`.
...
| Cast Q (v1) | Cast_q / CastUpper_q (`CastVariant`=1) | `atk_jump.fbx` (`skill1`) | `HeroAbilities.cs:618` `PlayCast(1)` | on-disk verified |
| Cast W (v2) | Cast_w / CastUpper_w (`CastVariant`=2) | `atk_shieldcharge.fbx` (`skill2`) | `HeroAbilities.cs:618` `PlayCast(2)` | on-disk verified |
| Cast E (v3) | Cast_e / CastUpper_e (`CastVariant`=3) | `f-ss-magespellcast-05.fbx` (`castHeal`) | `HeroAbilities.cs:618` `PlayCast(3)` | on-disk verified |
...
4. **CLOSED (STALE-FIXED 2026-09-06) - Knight W/E/R DO carry ids** (`knight.w/e/r`, minted by WO-750). The text below is the retired 2026-07-19 wording. Addressable by class+slot but NOT `FindById`. Owner ruling: mint `knight.w`/`knight.e`/`knight.r` (NOT `knight.shield-bash`, a distinct pool ability) into both JSON mirrors, or present by slot-address. Design docs favor slot-addressing for fixed kits.
...
*Fully resolved this pass (no longer gaps): ATTACK-pill damage = **52.5** (`_baseDamage`, in source at `PlayerAttackController.cs:43`, no prefab override); hot-swap chooser pool = exactly 16 ids; `weaponskill-animations.json` confirmed dead for Grom; `KnightMocap` 8/9 registry slots current.*

## Owner naming + redesign rulings (2026-07-19)

> **FROZEN, 2026-09-06 (Sunday sweep):** this section and the WO-750 SHIPPED / STILL OPEN blocks
> below are a dated point-in-time ledger (`CLAUDE.md` section 15: banner, never rewrite). Their
> clip names are the names AS OF 2026-07-19. **The live picks are in sections 1-3 above**, which
> were corrected against `motion-castings.json` on 2026-09-06.
...
| Q `knight.q` | **Sword Heroic** | `fist_flyingkick_m` *(the clip AS OF 2026-07-19; the live pick is `atk_jump` - see section 1)* | dash + 1s stun |
| W `knight.w` (minted) | **Shield Bash** *(renamed 2026-08-21)* | `fist_whirlwindkick_m` *(as of 2026-07-19; live pick `atk_shieldcharge`)* | cone knockback + slow + guard-break |
| E `knight.e` (minted) | **Warden's Grace** | **Mage Spell Cast 5** (owner-specified; supersedes the then-current `f-magiccontrol-01`) | REDESIGNED - see spec below |
| R/ult `knight.r` (minted) | **Radiant Strike** | **Jump into Slash Up** (owner-specified; replaces the then-current `fist_whirlwindkick_m` dup shared with W) | 220 dmg meteor |
...
- ~~Knight W/E/R carry no 'id'~~ **CLOSED - STALE-FIXED 2026-09-06.** All four ids exist in the tree (abilities.json:95/111/127/147 = knight.q/w/e/r; the kit _comment at :92 records the mint). This gap was closed by WO-750 and stayed open in this file for weeks; see the SHIPPED - WO-750 section above.
