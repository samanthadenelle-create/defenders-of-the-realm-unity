...
### WO-1221 — `resource panel expanded (opener live=True)` (shape H5 + H4) — ✅ **FIXED 2026-08-27** (bounce)

> **⚠ BOUNCE (owner felt-test 2026-08-27 FAIL).** The 2026-08-26 pass split the hollow line into
> REQUESTED vs VERIFIED PAINTED and called `_resExpandedRow.SetActive` from SetResourcePanelOpen.
> That was necessary and not sufficient. The expanded pixels still lived on a **second occupancy
> widget** (`resourceChips` / `_resDock`) that `hud-areas.json` never occupies. Occupancy is the
> only thing that turns a widget on (`Register` deactivates every widget at build). LateTick
> SetActive'd that WRAPPER — a full-ActionRail empty dock — so the tap still logged expand over
> `tmp/resources-expanded-105803.png` (gold 1034 only). Worse, `UiSurfaceProbe.MeasureRect` treats
> `activeInHierarchy=false` as a **named skip** (same bucket as batchmode), so the poll could not
> Fail the exact captured class.
>
> **✅ FIXED (WO-1221, 2026-08-27).** The four Wood/Iron/Stone/Crystals chips are now **children of
> the gold chip** (`resourceChipsCollapsed`, the occupancy-live opener). Toggle is SetActive on
> that child, from the tap, not a LateTick bool. `TickResourceExpandVerify` still polls
> `UiSurfaceProbe.MeasureRect` after layout settles and now **FAILS on INACTIVE** instead of
> skipping it. Verdict line remains `resource panel expand VERIFIED PAINTED — N/N rows measured
> on screen, panel kind=UGui rect=... opacity=... coveredBy=...`. Batchmode stays a named skip.

`Assets/_Modules/HUD/Kit/HudKitController.cs:~2895` (pre-fix)
...
`openerLive` is the predicate that produced this branch — `expand = openerLive && window`, so on the
expand path it is **True by construction** (shape H5), and the line is emitted on the activation frame,
before any layout pass (shape H4). What it actually proved: that a `SetActive(true)` call was made on
the widget WRAPPER. What shipped underneath it: `SetResourcePanelOpen` recorded a bool and nothing
anywhere in the tree re-activated `_resExpandedRow`, which WO-1205 had switched off at build time. For
the entire life of that build, every tap on the town gold chip logged `resource panel expanded (opener
live=True)` and painted **zero pixels** — the owner reported it as *"cannot click 1015 to see the
resources"*, i.e. the trace actively steered triage toward the input layer and away from the bug. The
corroborating trap: the same resources rendered fine in Build mode from a **different widget in a
different module** (`Village/BuildMode/BuildHudController.BuildResourceStrip`), which made the data
look healthy.
...
| ✅ **FIXED 2026-08-27 (WO-1221 bounce)** — `Assets/_Modules/HUD/Kit/HudKitController.cs` SetResourcePanelOpen / TickResourceExpandVerify | `"resource panel " + (expand ? "expanded" : "collapsed") + " (opener live=" + openerLive + ")"` | the town resource rail expanded and the player can see Wood / Iron / Stone / Crystals | 2026-08-26: that `SetActive(true)` was called on the widget **wrapper**. `openerLive` True by construction, pre-layout. 2026-08-27: the wrapper was the **wrong object** — `resourceChips` is not in occupancy, so ApplyPosture killed it before render; MeasureRect reported INACTIVE as a skip | it **did** lie: tmp/resources-expanded-105803.png is gold-only inside the expand window. The 08-26 probe could still skip-as-green over that capture. Now the stack is a child of the gold chip, the tap calls SetResourcePanelOpen, and INACTIVE is a Fail. | **HIGH** |
