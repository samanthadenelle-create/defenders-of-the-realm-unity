...
### WO-1221 — `resource panel expanded (opener live=True)` (shape H5 + H4) — ✅ **FIXED 2026-08-26**

> **✅ FIXED (WO-1221, 2026-08-26).** The line is kept, in place, on its `[Flow:HudKit]` tag, and
> **split into a request and a verdict**: `resource panel expand REQUESTED (...) — NOT yet a claim
> that anything painted` on the flip, then `resource panel expand VERIFIED PAINTED — 4/4 rows
> measured on screen, panel kind=UGui rect=... opacity=... coveredBy=...` from
> `HudKitController.TickResourceExpandVerify`, which polls up to 8 frames for layout to settle and
> routes every measurement through the shared `DeNelle.Core.Diagnostics.UiSurfaceProbe` (new
> `MeasureRect` entry point, for sub-surfaces that ride somebody else's canvas). Failure now has
> four named shapes from the probe plus a fifth of its own, `ROWS_MISSING`, which is the one that
> catches THIS defect: the rail's own rect can measure perfectly while every row inside it is
> inactive. Batchmode emits the probe's **named skip**, never a pass.
...
| ✅ **FIXED 2026-08-26 (WO-1221)** — `Assets/_Modules/HUD/Kit/HudKitController.cs:2895` | `"resource panel " + (expand ? "expanded" : "collapsed") + " (opener live=" + openerLive + ")"` | the town resource rail expanded and the player can see Wood / Iron / Stone / Crystals | that `SetActive(true)` was called on the widget **wrapper**. `expand = openerLive && window`, so `openerLive` is **True by construction** on this branch, and the read is taken on the activation frame, pre-layout | it **did** lie, for the whole life of the WO-1205 build: the panel `_resExpandedRow` was switched off at build time and re-activated by nobody, so every tap printed `expanded (opener live=True)` over an empty full-screen dock. Fires on **every resource tap in town** — the player's only route to their own balances — and it pointed triage at the input layer, where three separate `ClampMinTouch` investigations were spent. | **HIGH** |
