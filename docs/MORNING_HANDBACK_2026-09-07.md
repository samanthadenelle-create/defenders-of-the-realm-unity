...
- WebGL -> Vercel: LIVE. `WEBGL_BUILD_OK` (Builds/WebGL, 180 MB, 05:26), `R2_PUSH_OK 4 uploaded` + `R2_PARITY_OK objects=276` (05:27), chain deploy `dpl_3PgXC912YPSqegJotVq2KpKoff5N` -> production alias matched (05:4x); then a production REDEPLOY with the legal pages (`Builds/vercel-deploy-prod-redeploy.txt`), so `/privacy` and `/terms` answer 200; echoes-of-elarion deployed and its public domain aliased to that deployment; the dormant defenders-webgl domain aliased to the same bytes. `WEB_LEGAL_OK checks=4`, `WEB_PARITY_OK surfaces=defenders-of-the-realm-v2,echoes-of-elarion sha=eafe78a9...` (Builds/web-parity.log 06:0x). Both domains serve build 2026.09.07.359089. `/api/auth/nonce` answers 200 on the new deploy. NOT run: the chain's step-7 production DB write proof (the chain refused at step 6 before it; the api at HEAD is unchanged from what was already live, and the held renewal-cap files were stashed out of both uploads and restored after). Chain defects found on the way, each ticketed: a WebGL-only compile error the desktop gate cannot see (fixed, WO-1575 for the gate), the failed content build wiping the WebGL Addressables state so parity refuses before the build that restores it, web-ship deploying echoes-of-elarion without promoting the public domain, and the chain deployment shipping without the legal pages that web-ship copies in later.
