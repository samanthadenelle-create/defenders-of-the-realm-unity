...
> - **1200** (CLI main line — owner-minted, banner 1200→1201 per CLI) = seat-mail UI→CLI return path. IMPLEMENTED + locally verified: `seat-mail/seatmail.py` (queue core, selftest-green) + PowerShell reader hooks + `.claude/settings.json` wiring + `WORK_ORDER_1200_..RESULT.md`. **Transport: case (b), LIVE.** A bare `git push` 403 was misread as case (c); it was git-LFS failing on unpushable LFS objects (aborts the whole push), NOT a repo-write block — `GIT_LFS_SKIP_PUSH=1` pushes fine, and MCP `push_files` works too (only `create_branch` + SendMessage UI→CLI are actually blocked). Ref `seat-mail/ui-to-cli` live on origin (head `0c3299e4`, 4 real messages); verified end-to-end (fetch → surface oldest, pending>1 → ack decrements). Acceptance 3 (idle-CLI rewake) still needs CLI-side verification once the hooks merge onto `wip`.
> - **1192** (CLI main line — Rumor Board) = DESIGN DELIVERED: `WorkOrders/WORK_ORDER_1192_rumor_board_reward_slab.DESIGN.md`. Owner pivot (2026-08-25, FOUNDATIONAL_RULINGS above §12, supersedes §11): the slab shows the quest REWARD as icon+quantity (reuses `CurrencyChip`; showcase for the WO-1195/§13 fix), NOT an illustration — retires the 24-image program. Grounded in `quests.json`: 5 reward shapes (resource/item/keystone/mixed/progression-only); never-blank edge case (`forgemasters_act1` all-zero → "Unlocks: Act 2"). Two owner gates; flag: `magic` has no concept-icon. Layout fixes folded from the CLI's capture read (LFS PNG unviewable here).
> - Item **1194** (resource readout) HELD only for the ticket text (unreachable). Owner rulings 2026-08-25 removed both design deps: GOLD is excluded from the ambient dock (§13 cover) and shown contextually with the economy/resource/transaction UI; CRYSTALS stay UNCAPPED (premium currency — any backend ceiling must be effectively unreachable and never presented as a gameplay cap). §13 keeps the ambient posture dock OUT OF SCOPE.
> ---
> **OWNER RULINGS 2026-08-25 (locked into the specs above):** 1083 = (d) desktop-only letter (mobile drops badge, name owns row). 1192 = terminal reward (not aggregate); empty reward HIDES the row (no 0/None/shell); `magic` = MAP an existing library sprite in concept-icons.json (not new art, not text). 1195 = tight cost lines are icon+amount (names live in tooltips/onboarding); word-spelling formatters restricted to prose/non-mechanical surfaces only. 1194 = gold contextual (not ambient dock); crystals uncapped.
