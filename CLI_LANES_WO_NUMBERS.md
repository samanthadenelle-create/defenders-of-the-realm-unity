...
> - **1200** (CLI main line — owner-minted, banner 1200→1201 per CLI) = seat-mail UI→CLI return path. IMPLEMENTED + locally verified: `seat-mail/seatmail.py` (queue core, selftest-green) + PowerShell reader hooks + `.claude/settings.json` wiring + `WORK_ORDER_1200_..RESULT.md`. **Transport: case (c) at author time (git push 403 + MCP write 403 + SendMessage 403), flipped to LIVE case (b) after the owner authorized write.** Ref `seat-mail/ui-to-cli` pushed via MCP (commit 7d43ed3, 2 real messages); verified end-to-end (fetch → surface oldest, pending=2 → ack → pending=1). Acceptance 3 (idle-CLI rewake) still needs CLI-side verification once the hooks merge onto `wip`.
