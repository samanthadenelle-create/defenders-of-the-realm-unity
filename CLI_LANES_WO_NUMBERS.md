...
> ## ⚠ UI-SEAT MINT 2026-08-25 (on `claude/ui-spacing-layout-review-bqas0h`, CROSS-UNIVERSE — see note): two DESIGN specs authored for items the CLI seat (eoa-40) routed from its `wip/village2-and-f8-tickets` universe (WO 1100s), NOT this branch's local 781 lane. **CLI: reconcile these two numbers against your `wip` authority before adopting.**
> - **1195** = "a resource is named by its ICON, never a letter" — DESIGN + image briefs supplied into the CLI's existing WO-1195 ticket. Reuse `ElarionUiKit.CurrencyChip` (the HUD Resources-tab chip) across the 6 letter formatters + the `BuildWalletRow` "WIS" strip; enumerate (no hardcoded resource list); Image+Label (no TMP `<sprite>` pipeline exists). File `WorkOrders/WORK_ORDER_1195_a_resource_is_named_by_its_icon_never_a_letter.DESIGN.md`. Opens a small owner gate (icon-only vs icon+name in cost lines; word-spelling family scope; Wisdom concept-icon).
> - **1196 (PROVISIONAL)** = Class-select slot badge letterless — honour the 2026-07-19 "mobile HUD shows NO key-letters" ruling on `HeroSelectController.BuildSkillRow` (the sole remaining letter-paint site). Opens an owner gate over 4 candidates; UI-seat recommends (d) desktop-only letter (preserves the EXE case, satisfies mobile ruling). File `WorkOrders/WORK_ORDER_1196_class_select_slot_badge_letterless.DESIGN.md`. **No CLI ticket existed; 1196 is the next-free after the CLI-cited 1192/1194/1195 block — confirm before minting.**
> - Items **1192** (Rumor Board redesign) and **1194** (second-bank capacity readout) are HELD by the UI seat: their evidence (ticket files, `RumorBoard_*.png` captures, `QUEST_ILLUSTRATION_BRIEF.md`) is unpushed + LFS and unreachable from the cloud clone. Awaiting a push from the CLI seat.
