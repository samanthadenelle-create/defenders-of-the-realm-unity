...
> ## ⚠ REFRESHED 2026-07-18: next free WO = **745**. (**744** = strict-MVVM whole-game UI migration — every View binds an `IPanelViewModel`, ZERO runtime game-state reads; a conformance-oracle ratchet (`UiMvvmConformanceRegression`, wired into DataRegression as `[ui-mvvm]`) + **6 of 7 silos landed** on `wip` (B Shop/Equip/Inv + `GearIconCatalog` · C Build/Tower + repair-button fix · D Arena/Raid/Rumor · E Quests/Social/Music · F Echo/Harvest/Crafting · G-safe DungeonHud/LevelUp/Camp/JupiterSwap); **~33 views on VMs, oracle debt 28→16**, all §2c-tested + gate-green; **BattleHud + Dialogue landmines PENDING** (flag-gated, need owner felt-verify); spec `docs/UI_MVVM_MIGRATION_PLAN.md`.) (**740–743** = Room Forge + DungeonBaker into mainline — program `WORK_ORDER_PROGRAM_740_743_room_forge_into_mainline.md`: **740 merge DONE · 741 default-prefabs DONE** · 742 bake-demo + 743 canon-close remaining.)
> ## ⚠ REFRESHED 2026-07-17b: next free WO = **740** (SUPERSEDED by 2026-07-18 — 740–744 now consumed). (**739** = Generic Obsidian building-upgrade tier panel (Enhancement Path) — ONE data-driven panel for all 6 building ids (lumbermill/windmill/forge/armorer/arcane-tower/barracks-upgrade-tab), tier trees from `docs/design/BUILDING_UPGRADE_TREES.md` rev 2, VM binding map pinned by owner, adds `costIron`, mobile-first NO hotkeys; mockup `docs/UI_Mockups/building_upgrade_obsidian_template.html`; file `WORK_ORDER_739_generic_obsidian_upgrade_tier_panel.md` — READY TO IMPLEMENT.)
