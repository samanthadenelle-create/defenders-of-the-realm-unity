...
# Pair with f8-watch-daemon.ps1 (persistent, never exits). Re-launch this after each triage.
param(
    [int]$PollSeconds = 5,
    [int]$MaxMinutes = 0   # 0 = infinite
)
...
Write-Host "[f8-poll] armed poll=${PollSeconds}s inbox=$Latest"
