...
> ## ⚠ PROJECT NOTE (added 2026-08-23, CLI) — read before using this on Defenders of the Realm
>
> **Why this skill is installed:** the MON002 mainnet 1-SKR canary is fail-closed because the
> treasury recipient we have (`2VePaneS3xX2EdzSbe4JdiovRffboLJV4yNVmVTkeuCg`) is a **plain
> on-curve wallet, not a Squads vault** — verified on chain: it is ed25519-on-curve and
> System-Program-owned, whereas a multisig vault is program-derived and therefore **off**-curve.
> That is acceptable to prove the RAIL with 1 SKR; it is ⛔ **never acceptable as the production
> treasury**. See `docs/MONETIZATION_STATE_2026-08-23.md`.
>
> ### ⛔ THIS SKILL IS KNOWLEDGE, NOT AUTHORITY.
> It teaches the protocol. It does **not** authorise an agent to create, fund, or transact a
> treasury on the owner's behalf. Every on-chain step is signed by **her wallet**, by **her**.
> An agent's job here is to prepare, explain and verify — never to move funds.
>
> ### The project's own invariants still win over anything below
> - **Read the amount off the MINT, before the first transaction — never from a doc.** Mainnet SKR
>   is **6 decimals**; our Devnet test mint is **9**. A spec that carried the 9 into the mainnet row
>   would have moved **1,000 SKR** on a row meant to move exactly 1, and `/verify` runs AFTER
>   settlement, so the guard would have failed with the money already gone.
> - **The SKR token account does not exist yet.** Create it as a deliberate, funded step — do not
>   let a transfer create it incidentally.
> - **Verify the ATA on chain** (exists, holds the official mint, authority is the intended owner)
>   before it is authored anywhere. Classic token program `TokenkegQfeZ...`, **not** Token-2022.
> - The recipient must be authored in **both** canonical `wallets.json` mirrors **and** the server
>   environment, or client and backend will disagree.
> - **Cancel-first rehearsal**: decline at the wallet prompt and prove nothing moved, before the one
>   real 1-SKR transaction.
>
> Installed from `sendaifun/skills` (skill author `raunit-dev`), also listed in Solana's official
> skills directory. It is reference material — treat its code as examples to adapt, not to run as-is.
