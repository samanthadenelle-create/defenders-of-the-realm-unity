...
> below is a dated point-in-time snapshot, and per CLAUDE.md sec.15 dated ledgers are banner-fixed,
> never rewritten. The lag is not a maintenance failure that can be fixed by editing harder - it is
> what the file IS. So this banner stops pretending otherwise: **treat the body as archaeology.**
>
> **Where current state actually lives - go here, not below:**
>
> | Fact | Authority (read it, never copy it) |
> |---|---|
> | Everything current | `CANON_GROUND_TRUTH_<newest date>.md` at repo root |
> | Save schema version | `SaveSchema.CurrentVersion` (`Assets/_Modules/Core/State/SaveSchema.cs`) |
> | Feature flag defaults | `Assets/_Modules/Core/FeatureFlags.cs` |
...
> | Gate results | the MARKER on a FRESH log, never a doc and never an exit code |
...
> ---
>
> ## VERIFIED SNAPSHOT - 2026-08-21 23:0x (this is itself a dated entry and will go stale)
>
> Recorded here only so a reader who lands on this file is not stranded. Each line was read at
> source tonight, not copied from a doc.
>
> - **Branch** `wip/village2-and-f8-tickets`. **49 commits dated 2026-08-21**, of which **13 are
>   tonight's lane batch** (22:46-23:02). **`git rev-list origin/wip..HEAD` = 49 - NOTHING IS
>   PUSHED.** Do not assume any of tonight's work is on the remote.
> - **Save schema `v38`** (`SaveSchema.cs`, read at source). Nothing today bumped it.
> - **Gates:** `COMPILE_GATE_OK` present on a fresh log (`Builds/gate-final.log`, 22:42).
>   `DataRegression` = **245/247**, and the log carries **`REGRESSION_FAIL: 2 failures`** -
>   **`REGRESSION_OK` IS ABSENT** (`Builds/reg-final.log`, 22:44). Marker absence is a FAILURE,
>   not an unknown (CLAUDE.md sec.16). The two failures are ticketed ASSET gaps, not code:
>   **WO-1135** (wall tier materials were never tracked) and **WO-1136** (`staff_A` is
>   geometrically symmetrical, so no sheathe orientation is derivable). Neither is fixable in code.
> - **Build:** `Builds/Android/DefendersOfTheRealm.apk`, **546.7 MB** (573,230,639 bytes, 23:14).
>   **`R2_PARITY_OK 42 object(s) verified`** on `Builds/r2-parity.log` (23:16) - content is proven
>   hosted, so no capsule enemies (CLAUDE.md sec.16).
> - **STILL OFF, verified in `FeatureFlags.cs`:** `Siege` `defaultOn:false` (stays off until
>   **WO-1139** lands the ruled loss stakes - the cadence would otherwise open sieges that resolve
>   and report but take nothing). `RealmStorePurchase` `defaultOn:false` outside the
>   `STORE_RAIL_LOCAL_TEST` define; the mainnet block in `SolanaWalletProvider.SendPayment` is
>   **unlifted**. `MapTab` `defaultOn:false`.
> - **Shipped tonight:** Night Market store redesign (WO-1050) - PvE siege cadence + persisted
>   Defense Report (WO-1026, **DONE**) - per-camp raid cooldown + scaled attrition (WO-728) -
>   battle pass season track + monthly cards (WO-1053) - chest drops read by silhouette (WO-1132) -
>   convex Finish-Now curve + rescale parity (WO-1129) - per-mesh weapon seating - village cosmetic
>   seam + armorer instrumentation - realm map pins, dungeon status, offline accrual trust -
>   enemy art pipeline.
> - **WO-838 CLOSED** (owner felt-verified). **WO-1139 minted** (implement the ruled loss stakes).
>
> ---
>
> ## THE CORRECTION THAT CHANGES HOW YOU PRICE RISK (owner, 2026-08-21)
>
...
>
> The app IS live on the Solana dApp Store (the next submission is an UPDATE, not a new listing),
> and **nobody has ever bought anything.** "Published on a store" and "taking money" are DIFFERENT
> facts, and this repo's canon has stated the first loudly while the second has never been true.
>
> Consequence: **currency and economy REMOVALS are clean purges, not balance-preserving
> migrations** - there is nobody to grandfather or compensate. Still read-migrate a removed save
> field so existing dev/test saves LOAD (ordinary defensive deserialisation, not value
> preservation).
>
...
> ---
>
> **Reconciliation rule for the body below (recurring trap):** several systems are *more built*
> than their specs assume - the store, animation, and monetization were all near-complete already.
> Reconcile against the anchor and the code, never greenfield from a block below.
...
> ## SUPERSEDED 2026-08-21 - the block below is a 2026-08-06 snapshot, kept frozen.
> It still calls itself "CURRENT STATE" because dated ledgers are banner-fixed, not rewritten
> (CLAUDE.md sec.15). It is not current. Its schema (v36), its gate counts, its WO numbers and its
> push state are all frozen history. Current reality = `CANON_GROUND_TRUTH_2026-08-21.md`.

## CURRENT STATE - 2026-08-06 (SUPERSEDED 2026-08-21 - frozen; anchor = `CANON_GROUND_TRUTH_2026-08-06.md`)
