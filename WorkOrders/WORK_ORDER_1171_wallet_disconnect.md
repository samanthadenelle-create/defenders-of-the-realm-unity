...
**Status:** FIXED 2026-08-25 - **section 4 has now landed**, so both halves are code-complete; **awaiting owner felt-verify** (CLAUDE.md section 13: the PO closes, not the CLI). Dev-panel surface = `d1b6239ca` (2026-08-24). Player-facing placement = `Assets/_Modules/Settings/SettingsController.cs` lines 248-260, commented `// WO-1171 section 4`: a "Wallet" caption carrying BOTH `Connect Wallet` and `Disconnect Wallet`, routed through `CurrencySkinResolver.RequestWalletConnect` / `RequestWalletDisconnect` (so the Wallet assembly stays behind the Core seam - no asmdef change) and kept live by `OnWalletConnectionChanged` -> `RefreshWalletControls` (line 474). The section-4 MECHANISM (a player-selectable preference chain) was split out to **WO-1196**, which has also landed. Gate: `COMPILE_GATE_OK` + `REGRESSION_OK 279/279 suites, 0 red` on fresh logs (`Builds/gate-final2.log`, `Builds/reg-final2.log`). *(Board reconcile note: the in-body section-5 checkboxes still read unchecked - body untouched by the board keeper; the section-4 box is satisfied by the code cited above, the owner felt-test box is genuinely still open.)* *(Prior bucket line preserved below.)*
