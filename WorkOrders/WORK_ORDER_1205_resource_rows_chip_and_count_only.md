...
**Status:** FIXED 2026-08-26 — the ruled resource-row implementation landed in `5825ef671`: the town HUD rests collapsed, expanded rows render `[icon] count` without `" of "` or the word label on the art-resolved path, icon and digits occupy disjoint sub-rects, the existing `CurrencyChip` tag restores the resource name when art is unavailable, and the separately ruled build-screen strip remains untouched. The later shared-surface follow-up in `b303c4fbf`/`bee577cf7` closes the discovered invisible-expansion defect by routing the toggle through `SetResourcePanelOpen`, activating `_resExpandedRow`, and measuring the painted rows after layout; it also implements the owner-ruled persistent toggle and collapsed Gold `+4` hint. All commits are ancestors of APK source `bcef3be7`. Fresh 2026-08-26 gates report `COMPILE_GATE_OK` and `REGRESSION_OK 292/292 suites -- 292 green, 0 red, 0 skipped`; `CollectorTellRegression` pins removal of cap text, the icon/digit split and no-art identity fallback, while `[hud-ui-sme]` reports the resource-rail raise and measured-expand verification green. Awaiting owner felt-test at device resolution, including opened-PNG overlap and greyscale silhouette judgement, to close.
