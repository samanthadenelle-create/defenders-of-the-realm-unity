...
**Status:** CLOSED 2026-08-23 — CLI-VERIFIED FROM DATA, not felt-tested, and that is the correct route for this ticket (owner: *"1038 is something you need to verify"*, *"i dont get errors"*). ⛔ **THE OWNER'S INSTINCT IS THE POINT: an undeclared tag throws into the LOG, never onto the screen, so "I don't get errors" is the expected symptom whether it is fixed or not. Player eyes cannot settle this class.** Verified on two independent channels against the WO-1159 go-live build: **(1) STATIC at HEAD** — `TagManager.asset` still declares exactly four tags (`Tower`/`Building`/`HeartTarget`/`Player`), so all five tags remain undeclared and the WO's FACTS stand; but every live read now routes through `SafeFindWithTag`, the `SpawnPoint` site that minted this ticket is gone (comment only at `CastleDefensePlansService.cs:268`), `Pet` likewise (`PetHarvestBootstrap.cs:210`), and the ONE remaining raw `GameObject.FindWithTag("HeroTarget")` (`CastleNavTopologyDiag.cs:100`) sits INSIDE `Guard.Try(Sys, "hero-fallback-tag", ...)` so it cannot throw uncaught — **stronger than the 2026-08-17 VERIFICATION doc, which called that site "dormant behind a dev-only flag"; it is in fact guarded.** **(2) RUNTIME from the owner's live session** — `adb logcat -d -t 4000`: **0 `UnityException`, 0 "is not defined", 0 `NullReferenceException`**; and F8 across the entire session reads **`NO_CAPTURE` ack=3607**, which covers boot too (`FlowTrace.Fail` → `LogError` → harness capture), closing the logcat ring-buffer blind spot recorded in memory `logcat-ring-buffer-destroys-evidence`. ⚠ **NO TagManager EDIT WAS MADE, DELIBERATELY** — the VERIFICATION doc §4 shows adding these tags makes things strictly worse. The WO's headline severity ("each is a live UnityException", "13 live exception sites") was and remains **FALSE**; its facts were **TRUE**. Prior status: FIXED — AWAITING OWNER FELT-TEST TO CLOSE / DONE — audit-verified as shipped (2026-08-21 backlog audit).
