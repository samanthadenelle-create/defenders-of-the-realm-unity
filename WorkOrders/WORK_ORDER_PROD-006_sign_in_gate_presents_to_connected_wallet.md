...
**Status:** CLOSED 2026-08-23 — OWNER FELT-VERIFIED on the go-live Seeker build (owner: *"prod 06 works"*). ⭐ **Load-bearing on THIS build specifically, more than on the one it was fixed against:** WO-1159 made the wallet both the identity key and the payment rail, so a sign-in gate that ignored a connected wallet would now stand between the player and a purchase, not merely annoy them at launch. The identity law it enforces — only the wallet path re-keys the save, email/Google binds nothing — is what makes `ShouldContinueWithoutLogin(...)` correct rather than a timing workaround; no timing constant was ever added, and `LoginGateRegression` (`[login-gate]`) holds it. Prior status: FIXED — AWAITING OWNER FELT-TEST TO CLOSE / IMPLEMENTED 2026-08-18 (`ba76f67eb`) — VERIFIED ON DEVICE, build 331367: auto-resume succeeded and there is NO `LoginPanelController:Build` line anywhere in the session, i.e. the panel never constructed (`docs/proof/2026-08-18-overnight-gear-structures/README.md`). Mechanism confirmed live at HEAD (`LoginPanelController.cs:113-115`, `:154-173`; `GameStateService.cs:1308`) and guarded by `[login-gate]`, registered at `DataRegression.cs:479`. ⚠ NOT PROVEN: §6.2, the clean-install PRESENT path, was never run on a device — only in the regression truth table. — AWAITING OWNER FELT-VERIFY (PO closes, §13).
