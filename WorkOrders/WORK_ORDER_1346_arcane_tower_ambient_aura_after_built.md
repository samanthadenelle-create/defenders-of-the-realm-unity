...
**Status:** CLOSED 2026-09-06 - owner felt-test PASS (validated 2026-09-04T17:25:57, build 2026.09.04.354315). PRIOR STATUS: FIXED 2026-09-03 - shipped in 2026.09.03.353999. ArcaneTower_Aura wired to the FIRING Spire (tower_arcane_spire), REPLACING Aura_HeartPulse on the ONE existing ArcaneAura component - a separate component would have been re-Ensured onto the tower at the next upgrade by StructureFactory.ReskinForLevel, giving two ambient auras on one structure. Gated on BUILT state via a DERIVED predicate (a live scaffold), never the completion event, so an already-built tower lights on a cold reload. Tears down on destruction through the existing Destructible path. "Softly" = 45% emission on the SPAWNED INSTANCE (density, not alpha or hue), tunable-seam-ready and answering the built-in default until the six sources land. AuraOverArcaneTower_Aura has ZERO code consumers - nothing to remove, no second spawner. NOTE for her: ApplyLevel also applies a pre-existing tier scale (2.2/3.0/3.8), so her scale 1.0 is multiplied by tier - flagged, unchanged.
