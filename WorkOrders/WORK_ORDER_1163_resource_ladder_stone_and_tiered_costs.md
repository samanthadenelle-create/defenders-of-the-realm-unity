...
**Status:** READY - OFFTREE RETURNED lane=codex/wo1163-r2 2026-08-25 WITH A MONEY BUG. Nothing has landed on this branch: verified at HEAD that neither canonical `packs.json` copy carries a `stone` key, so this ticket is takeable from its spec, not from a half-built tree. The revision moved every surface the MIRROR LAW named, produced 31/31 quote + 57/57 backend suites, was correct on its own terms - and would still have granted ZERO on three live impulse SKUs, because `packs.json` authors `stone`, `PackCatalog.PackEconomy` has no such binding, and Newtonsoft drops the unbound key SILENTLY (`MissingMemberHandling.Ignore`, `PackCatalog.cs:654`). The AMENDMENT at the foot of this file raises the MIRROR LAW from FOUR surfaces to FIVE - the fifth being the Unity client, where the player's money actually turns into resources - rules that a green node suite is NOT evidence surface 5 moved, and adds an ORACLE DEBT for it. PRIOR STATUS: (2026-08-24) UNBLOCKED - WO-1177 landed (`2c3ed6c24`) and released the shared `api/_lib/purchase-catalog.js` / `test/purchases.quote.test.js` lock, and the owner ruled the food SKU ids DO rename, so the remap is IN SCOPE here rather than a follow-up. Section 6 fully answered (6.1 + 6.2 ruled 2026-08-24, 6.3 ruled 08-23). This is still a save-schema-adjacent change on a LIVE build with an ACTIVATED pay path (WO-1159) - follow section 7's sequencing exactly; not a lane to rush.
