...
**Status:** READY - PARTIAL (board reconcile 2026-08-25). **Sites 1, 2 and 3 are DONE; site 6 is WITHDRAWN as mis-specified; sites 4 and 5 remain OPEN**, so the ticket stays in Ready. Site 1 = `f6e306847` (`Assets/Editor/TowerPerkFallbackGenerator.cs` -> `Village/Buildings/Generated/TowerPerkFallbackData.g.cs`). Sites 2 + 3 = `10912de95`: `Assets/_Modules/Village/Catalog/Generated/BuildCategoryFallbackData.g.cs` + `Assets/_Modules/Core/Platform/Generated/StakeRewardsFallbackData.g.cs`, produced by `Assets/Editor/BuildCategoryFallbackGenerator.cs` / `StakeRewardsFallbackGenerator.cs` and hash-gated by the shared `Assets/Editor/Regression/GeneratedFallbackParityRegression.cs` (registered in `DataRegression.cs`). **Site 6 (enemy VFX) is WITHDRAWN, not done** - no canonical VFX JSON exists to generate from, and deleting the synthesized rung would turn a visible telegraph into a missing combat cue (ruling recorded in `BATCH_STATE.md`). **RESIDUAL:** site 4 `Assets/_Modules/Onboarding/HeroCatalog.cs` (the "hand-mirror kept in sync" comment is still at line 156 of HEAD), site 5 `Assets/_Modules/Village/Enemies/OverworldEncounterSpawner.cs`, and the section-6 standing oracle for new JSON-mirroring literals. Gate at this reconcile: `COMPILE_GATE_OK` (`Builds/gate-final2.log`) + `REGRESSION_OK 279/279 suites -- 279 green, 0 red` (`Builds/reg-final2.log`).
