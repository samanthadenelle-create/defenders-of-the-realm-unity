...
**Status:** FIXED 2026-08-25 (`10912de95`) - the mechanism landed; **awaiting owner felt-verify**, with one acceptance item NOT proven by a suite (named below). New `Assets/_Modules/Wallet/WalletPreferenceStore.cs`: PlayerPrefs key `dotr.mwa.preferred_package.v1` (NOT the save schema, so no schema bump), `DefaultPackage = com.solanamobile.wallet` so a player who never chooses sees today's Seeker behaviour, and `TrySetPreferredPackage` accepts only a currently-installed handler, takes an explicit `kingdomSwitchConfirmed` flag (owner ruling option (a), `d2351ef04`) and clears the sealed grant on a real change. `Assets/_Modules/Wallet/TargetedLocalAssociationScenario.cs` consults `WalletPreferenceStore.StoredPackage` (line 572) BEFORE walking `PreferredWalletPackages` (line 587), which stays the fallback order. Gate: `COMPILE_GATE_OK` + `REGRESSION_OK 279/279 suites, 0 red` (`Builds/gate-final2.log`, `Builds/reg-final2.log`). **RESIDUAL / NOT VERIFIED BY THIS RECONCILE:** acceptance 1-4 say "prove it", and **no regression suite references `WalletPreferenceStore`** - at HEAD the only file that does is `TargetedLocalAssociationScenario.cs`. Resolution order and the seal-clear read as correct at source but are not pinned by a test, and the Android association path cannot be exercised headless.
