...
**Status:** FIXED 2026-08-25 (mechanism `10912de95`, oracle `5b375cb82`) - built AND now pinned by a suite; awaiting owner felt-verify. `Assets/_Modules/Wallet/WalletPreferenceStore.cs`: PlayerPrefs key `dotr.mwa.preferred_package.v1` (NOT the save schema, so no schema bump), `DefaultPackage = com.solanamobile.wallet` so a player who never chooses sees today's Seeker behaviour, and `TrySetPreferredPackage` accepts only a currently-installed handler, takes an explicit `kingdomSwitchConfirmed` flag (owner ruling option (a), `d2351ef04`) and clears the sealed grant on a real change. `TargetedLocalAssociationScenario.cs:572` consults `WalletPreferenceStore.StoredPackage` BEFORE walking `PreferredWalletPackages` (`:587`), which stays the fallback order. RESIDUAL CLOSED 2026-08-25: this line previously recorded that no regression suite referenced `WalletPreferenceStore`. One does now - `Assets/Editor/Regression/WalletSessionPersistenceRegression.cs` section 5 pins all three acceptance items against real PlayerPrefs: with no stored choice the rank-1 Seeker package wins as `chain rank 1` even when another capable wallet is first in the list; an installed stored choice wins as `stored choice`; and a confirmed switch both persists the package and leaves NO resumable sealed session. Wired into `DataRegression.RunAll` as `[wallet-session]`, and the run log names it - "Seeker default and stored-choice precedence pinned". Gate: `COMPILE_GATE_OK` + `REGRESSION_OK 279/279 suites, 0 red`. What a suite still cannot reach is the live Android association handshake, which does not run headless by construction - that is the owner's felt test, not a hole in the oracle.
