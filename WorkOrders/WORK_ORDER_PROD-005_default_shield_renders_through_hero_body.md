...
**Status:** CLOSED 2026-09-03 - owner felt-test PASS. PRIOR STATUS: FIXED — AWAITING OWNER FELT-TEST TO CLOSE. Prior status: IMPLEMENTED 2026-08-18 (`c072e5736` publishes the address; `228908bfc` restores the size) — PROVEN ON DEVICE BY MEASUREMENT, build 331367: the address resolves on hardware and the prop measures worldBounds y=0.92 m against legacy `shield_A`'s 0.918 m (`docs/proof/2026-08-18-overnight-gear-structures/README.md`). ⚠ NOT PROVEN: §5.3/§5.4 — the dungeon→town port was never exercised and no post-port screenshot exists, so THE ANCESTOR BUG ITSELF IS UNVERIFIED. ⚠ §5.6 WAS DELIBERATELY DEVIATED FROM — a `ShieldWithItemLogic` row WAS authored (scale-only, derived, rot/pos all zero); see the RESULT for why. — AWAITING OWNER FELT-VERIFY (PO closes, §13).
