...
**Status:** READY — **VERIFICATION ONLY. ⛔ NO IMPLEMENTATION AUTHORIZED — no developer edits the oracle.** Exactly **one** item remains, and it is a *proof run*, not a code change: **§5.2's known-bad (red) direction** — a controlled run showing `STRUCTURE_ORIENTATION_FAIL` actually fires against the **pre-fix catalog state**. ⚠ Without it a green run proves nothing: **a vacuous oracle is also green.**
⭐ **The fresh green run is DONE (2026-08-24, lead seat):** `Builds/gate-ladder.log` (2026-08-24 19:47) carries **`REGRESSION_OK 273/273 suites -- 273 green, 0 red, 0 skipped`**, with `StructureOrientationOracle` registered and green in that same log — `[structure-orientation] STRUCTURE_ORIENTATION_OK — 34 structure model(s) measured … 20 held height fidelity … 6 tower-class model(s) stood above the 1.2 upright-aspect floor`. That IS the current-tree run this ticket was waiting on; the earlier "7 issues down to 5" remainder is cleared.
*(Board note 2026-08-24 (second pass): bucket unchanged (Ready). The banner was re-led with a canonical token and narrowed — it is verification work, so the lane below must not be read as an invitation to change `StructureOrientationOracle.cs`. ⛔ `WORK_ORDER_PROD-008_orientation_height_oracle.RESULT.md` is FROZEN (CLAUDE.md §15) and was left untouched by this pass, even though its registration statement now reads stale.)*

**Prior banner, retained for history:** READY — PARTIAL. The oracle exists and measures correctly 2026-08-18 (`fc9b1eb69`) (`Assets/Editor/Regression/StructureOrientationOracle.cs`, markers `STRUCTURE_ORIENTATION_OK/_FAIL`). ⭐ **TWO OF THE THREE REMAINING ITEMS HAVE SINCE LANDED — verified at source 2026-08-24:** (1) §5.1 registration is **DONE** — it is invoked from `DataRegression.RunAll` (`Assets/Editor/Regression/DataRegression.cs:1154`) and the `regression-registry: standalone` opt-out is gone from its header; (2) the **owner ruling landed 2026-08-23** (`f1d2c5d9b`) — the ballista was ruled MIS-CLASSED, not mis-oriented (`heightMul` 1.2 → 0.75, joining catapult/siege tower; ⛔ the 1.2 tower floor was deliberately NOT widened and the art was NOT re-oriented), and both ballista entries cleared. **WHAT REMAINS:** the oracle was still red on other rows at that commit ("7 issues down to 5"), so a fresh lead-run is owed to say what is left; and (3) §5.2's known-bad direction was never run, so the gate is still proven in ONE direction only.
