**Status:** FIXED 2026-08-24 - awaiting owner felt-test. ⭐ **The slice this line called "not done" IS IN THE TREE and committed:** `BackendRequestSigner.WarmUpSessionAsync` (`Core/Web3/BackendRequestSigner.cs:245`) mints the session AT CONNECT from the existing cached proof, and it is called from **both** connect paths - `WalletSkinBootstrap.cs:218` and `:325` - so auto-resume is covered too. No second signature is needed at purchase.
>  ⚠ **Corrected twice on 2026-08-24, and the failure is worth keeping.** This line stayed stale after the fix landed; a review read the line and reported the work outstanding; I read the review and DOWNGRADED a ticket that was already done. Three readers trusted the same stale sentence. ⛔ Read the tree, not the status - especially when the status agrees with you.
