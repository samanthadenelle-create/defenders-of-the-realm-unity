...
**Status:** FIXED 2026-08-26 — the original missing-material diagnosis was disproven: `Portal_Threshold_Aura` resolves to `pf_vfx-ult_demo_psys_loop_portalBlue`, whose enabled drawable renderers retain their prefab-owned materials while its all-null renderers are authored-disabled vendor containers. The correction and registered `[vfx-null-slot]` oracle landed in `bb9844a97`, with the `renderMode=None` false-debt correction in `0082dcc99`; both are ancestors of APK source `bcef3be7`. Fresh `Builds/pre-seeker-regression.log` evidence reports `VFX_NULL_SLOT_OK` (172 prefabs checked, 0 unresolved) and `REGRESSION_OK 292/292 suites -- 292 green, 0 red, 0 skipped`. Owner device visual verification of the portal remains owed before closure.
