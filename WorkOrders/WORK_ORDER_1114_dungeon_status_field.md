...
**Status:** BLOCKED ON DEPLOY - **but on ONE remaining item, not three** (board reconcile 2026-08-25). Bucket UNCHANGED; the cited blockers were stale. **Blocker 1 is CLEARED:** `dungeon_status` now EXISTS in the live Neon database - `api/migrations/20260824_0001_repair_schema_parity.sql:6-7` was written specifically because "dungeon_status was absent", and the owner then ran the production parity check, marker `SCHEMA_PARITY_OK 17 table(s) verified against api/schema.sql` (recorded in `batch_results_state.md`). `api/schema.sql` declares exactly **17** tables and `dungeon_status` is one of them, so the green covers it. **Blocker 2 (`vercel --prod` for `api/dungeon-status.js`) was already conceded cleared** by the api production promotion `e2e07f1c0` in the prior line's own words. **THE ONE THING STILL OWED IS BLOCKER 3, and it is exactly the trap this ticket named:** there is still **no `dg_healers_cottage` and no `dg_folks_granary` seed row** - `api/schema.sql:902` seeds `dg_starter_loop` and its siblings under `ON CONFLICT (dungeon_id) DO NOTHING`, so re-running the file can NEVER add them (memory `idempotent-ddl-hides-a-stale-table`, the same shape that hid the missing CHECKs on the money path). Until those two rows exist, the two REAL shipped dungeons fall to the client default with no server row behind them. Nothing further is owed in the Unity tree: R1-R6 landed and the oracle is registered at `Assets/Editor/Regression/DataRegression.cs:718` (`dungeon-status suite`). *(Prior line preserved below.)*
